...
| `randdigit()` | A random digit `0`-`9`. Drawn from the RP2040's ADC: GPIO26 left floating as a noise source, XORed with the internal temperature sensor's LSB jitter and a free-running timer bit (to guarantee variation even if the ADC reads nearly identically on back-to-back samples), then [Von Neumann debiased](https://en.wikipedia.org/wiki/Randomness_extractor#Von_Neumann_extractor) and rejection-sampled into range. Genuinely non-deterministic hardware entropy — **not cryptographically secure** (no whitening beyond debiasing, no seed-stretching), but real randomness, not a PRNG. **Measured around ~100ms per call** when called repeatedly in a loop (250 back-to-back calls took ~30s) — noticeably slower than everything else in the shell, so budget for it in anything calling it many times. Every loop in `src/adc.c` is bounded with a fallback rather than able to hang indefinitely, even in a pathological worst case. |
