...
/* The first 256KB of flash is reserved for the kernel image itself (pico-sdk
 * links code/rodata/data there, same reservation convention as
 * src_2040/fs.c uses for its own kernel image, against this board's 16MB
 * flash instead of the RP2040 build's 8MB). */
#define FS_BASE_OFFSET   0x40000u
#define FS_FLASH_SIZE    (16u * 1024u * 1024u - FS_BASE_OFFSET)
